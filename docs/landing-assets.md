...
|---|---|---|---|
| `cutouts/` | `<part>-<w>.avif` + `.webp` | 480 · 768 · 1024 · 1440 | alpha preserved, **not** mirrored |
...
126 files, **5.9MB committed** (cutouts 2.3MB · plates 556KB · video 3.1MB).
Encoder settings: AVIF q55 effort 6, WebP q72 effort 5, metadata stripped.
Only `chapter-2` and `chapter-4` are encoded — 1 and 3 stay in `landing-src/`
as marketing material and never enter git.

`apps/web/lib/landing-assets.json` is the generated index: intrinsic size,
alpha, mirrored flag and per-width byte counts for every asset. It is the
machine-readable half of this table — regenerated by the same script, never
hand-edited.
