...
describe("POST/DELETE/GET /me/wishlist", () => {
  it("rejects every route without a session", async () => {
    ...
    const results = await Promise.all([
      ...
      fetch(`${baseUrl}/api/v1/me/wishlist/${product.id}`, { method: "POST" }),
      fetch(`${baseUrl}/api/v1/me/wishlist/${product.id}`, { method: "DELETE" }),
    ]);
  ...
  it("saves a product and lists it back, hydrated", async () => {
    ...
    const addRes = await fetch(`${baseUrl}/api/v1/me/wishlist/${product.id}`, {
      method: "POST",
      headers: customerCookie(userId),
    });
    expect(addRes.status).toBe(200);
    const addBody = (await addRes.json()) as Envelope<{ productId: string; isSaved: boolean }>;
    expect(addBody.data).toEqual({ productId: product.id, isSaved: true });
    ...
    expect(listBody.data[0]!.productId).toBe(product.id);
    expect(listBody.data[0]!.product.id).toBe(product.id);
  ...
  it("resolves the wholesale price for a wholesale account, same as the catalog list", async () => {
    ...
    await fetch(`${baseUrl}/api/v1/me/wishlist/${product.id}`, {
      method: "POST",
  ...
  it("is idempotent on a repeat add — no duplicate entries, no error", async () => {
    ...
    const secondAdd = await fetch(`${baseUrl}/api/v1/me/wishlist/${product.id}`, {
      method: "POST",
      headers: cookie,
    });
  ...
  it("removes a saved product, and is idempotent when it wasn't saved", async () => {
    ...
    const removeRes = await fetch(`${baseUrl}/api/v1/me/wishlist/${product.id}`, {
      method: "DELETE",
      headers: cookie,
    });
    ...
    const secondRemove = await fetch(`${baseUrl}/api/v1/me/wishlist/${product.id}`, {
      method: "DELETE",
      headers: cookie,
    });
  ...
  it("only ever returns the requesting user's own saved products", async () => {
    const userA = (await seedUser()).id;
    const userB = (await seedUser()).id;
    const product = await seedProduct();
