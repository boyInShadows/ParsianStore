/**
 * Persian text + money helpers (§7.5) with NO date dependency.
 *
 * Split out of fa.ts deliberately, and measured: fa.ts imports dayjs +
 * jalaliday at module scope for `formatJalali` alone, so any client
 * component that only needed `toPersianDigits` or `formatToman` was pulling
 * the whole date stack into its route bundle -- +6KB First Load JS on every
 * client-tree route the moment `Pagination` started rendering Persian page
 * numbers. Importing "schemas/fa-text" costs none of that.
 *
 * fa.ts re-exports everything here, so `import { formatToman } from "schemas"`
 * keeps working unchanged; reach for the subpath only from client components.
 */
