...
**Status:** Phases 0–4 complete (P0.S1–P4.S8). Palette refined to v1.5 (Marigold CTA) per owner review. **GATE 4→5 cleared 2026-07-28.** Phase 5 (storefront) executed incrementally, by the owner's explicit "one piece at a time" choice — **P5.S1 (Category/PLP), P5.S2 (PDP), P5.S3 (search results), P5.S7 (wishlist + `/auth/login`), and P5.S8 (cart) shipped; P5.S4–S6 (brand pages, vehicle landing pages, compare) were deprioritized in favor of cart and remain undone**, per the owner's own ordering, not an oversight — §13's table still carries their draft specs for whenever they're picked up. **GATE 5→6 cleared 2026-07-29.** Phase 6 (checkout/payments/orders) is directionally locked, still to be re-specced when its own turn comes — the owner is running it the same "one piece at a time" way as Phase 5. **P6.S1–P6.S7 are done and shipped — Phase 6 (checkout/payments/orders) is complete.** A real shopper can now go from PDP to a paid, optionally-discounted order end to end: B2B wholesale pricing, address book, `PaymentProvider` (Mock + Zarinpal sandbox), shipping zones & rates, Order + checkout initiation, the real checkout frontend, and coupons. Wallet (Phase 7 dashboard scope, not Phase 6) and pay-on-delivery (deferred at P6.S5's own kickoff) remain open by explicit design, not a gap. **Phase 7 (user dashboard) has now started, run the same "one piece at a time" way — P7.S1 (My Orders), P7.S2 (address book page — real add/edit/delete), P7.S3 (wishlist page), and P7.S4 (My Garage management page) are done and shipped — all three of Phase 7's original picks are now complete.** See §13's Phase 6/7 entries for the real, shipped detail per step. **Phase 8 (admin dashboard) has also now started — P8.S1 (admin order management: list, detail, manual status update), P8.S2 (admin products CRUD + inventory adjust UI) P8.S3 (admin coupon CRUD + wholesale-account-flag UI) and P8.S4 (admin categories/brands/attributes CRUD + the product-attribute link) are done and shipped**, the two most urgent Phase 8 gaps: staff previously had zero visibility into orders and no way to manage the catalog outside a raw seed script. A known, documented bundle-budget tradeoff came with P8.S1 — see §13's Phase 8 entries and the Changelog below.
**Document version:** 1.28 — Phases 0–4 are locked; Phase 5 is gate-cleared with P5.S4–S6 left as open backlog, not blocking Phase 6. Phase 6 is complete (P6.S1–P6.S7). Phase 7's original three-piece scope is complete (P7.S1–P7.S4 shipped). Phase 8 is underway (P8.S1–P8.S2 shipped), re-specced piece by piece as each one starts. **English (`en`) locale maintenance is suspended as of 2026-07-30 — owner confirmed directly the business won't sell in English; `en.json` parity is no longer part of any step's DoD, deferred to a future, lowest-priority backlog item (§7).** **UI/UX quality is now a standing concern (owner rated the recent Phase 6/7 pages 1/10 on 2026-07-30) — every new frontend step is expected to actively apply `web/design-quality.md`'s checklist, not just pass functional DoD; a dedicated design-pass revisiting already-shipped pages is a real, not-yet-scheduled backlog item.** **A real, accepted bundle-budget tradeoff exists as of P8.S1 (2026-07-30): the landing page runs 8KB over its own 180KB §10 budget (188KB), a side effect of `(admin)`'s real MUI content reshuffling webpack's shared-chunk graph app-wide — investigated thoroughly, two fixes attempted and failed, owner confirmed accepting it rather than continuing to chase it.** Phases 9–10 remain directionally locked, re-specced only when their turn comes.
...
**Changelog:**
- 1.28 — **§4 manifest amendment: `clsx` + `tailwind-merge` added to
  `apps/web`.** The only dependencies the Phase 11 design-system track
  adds, approved by the owner on 2026-08-26 alongside the decision to
  keep the locked stack (no Tailwind v4, shadcn/ui, Radix, Lucide or
  CVA). They back `lib/cn.ts`, which is what lets a caller's
  `className` actually override a primitive's base class rather than
  merely sit beside it. tailwind-merge is pinned to the **v2** line on
  purpose: v3 is built for Tailwind CSS v4, which §2.3 rejects.
  Rationale and the S1–S6 plan: `docs/decisions/0027-design-system-consolidation.md`.
- 1.27 — **Landing rebuild (Phase 9 track) adopted.** The owner supplied a set
  of rendered 3D assets and commissioned an external plan (Fable 5) against a
  repo context brief (`docs/landing-rebuild-brief.md`); the resulting plan
  (`fableTasks.md`) is adopted with the amendments recorded in §5's landing
  amendment and PHASE 9's step table below. Headlines: the landing section
  inventory drops **15 → 11**, Shop-by-system is absorbed into the hero, the
  Exploded View is re-skinned (not replaced) from flat SVG to layered
  transparent part renders driven by scroll, Numbers is cut and
  Newsletter/Guides are hidden until their data exists, and the whole rebuild
  ships with **zero new runtime dependencies** — `three`/`@react-three/fiber`
  stay out of the manifest and WebGL is parked as an explicit future owner
  decision. Verified before adoption, not assumed: the ten part renders are
  **real transparent cutouts** (2048², 74–90% fully transparent, corner alpha
  0), `lightning.png` is a sealed-beam headlight, and `car.png` is a classic
  fastback coupe — a placeholder for the domestic-sedan regeneration the owner
  already parked as a later drop-in swap.
...
- 1.18 — **P7.S3 (wishlist page) shipped, first step under the new UI/UX quality mandate.** The owner's third Phase-7 pick (the last of address-book/wishlist/My-Garage) — and, right after it was picked, the owner rated the recent run of Phase 6/7 pages 1/10 on visual quality, with explicit instruction to keep moving but stop letting design quality slide. This step's own answer: reuse `plp/ProductCard.tsx` (the PLP/search grid's own, already-better-designed component) instead of inventing another bordered-row list like `/orders`/`/addresses` — `/wishlist` renders the same grid `/search` does, inheriting its established visual language for free rather than adding a fourth flat-list pattern. A real backend gap surfaced building this: `GET /me/wishlist` (P5.S7) had only ever needed to hydrate product data for the *toggle button's* own state, so it returned the raw Mongoose product doc — missing `isWholesalePrice`, and leaking the raw `wholesalePriceRial` field to every viewer regardless of account type. Fixed by routing it through `pricing.ts`'s `toPublicProductJson` (P6.S1's own "the one place effective price is decided"), the same shaping every other product-serving list (`/catalog/products`, `/catalog/search`, related) already uses — 2 new backend tests cover the fix (shape assertion + a real wholesale-price resolution case). `EmptyState` (`components/primitives`) gained an optional `icon` slot, additive and backward-compatible (15 existing call sites unaffected), used here with the same heart glyph `WishlistButton` already established — a small, deliberate first move on the design-quality mandate rather than a token-only page. `AccountNav`'s `active` union grew a third value; `/orders` and `/addresses` both updated to pass the new required label. `/wishlist` uses `/orders`' server-side auth gate (zero page-level interactivity beyond what each card's own `WishlistButton` already ships independently). 401/401 backend tests (2 new). Route budget: `/wishlist` 127KB First Load JS, matching `/search`'s own footprint exactly (same shared `ProductCard`/`WishlistButton` client bundle, no extra JS from the page itself). See `docs/decisions/0018-p7s3-wishlist-page.md` and §13's Phase 7 entry for full detail.
- 1.19 — **P8.S1 (admin order management) shipped — Phase 8 has started.** Owner-confirmed scope before writing code: status workflow only (refunds/invoices deferred, no `PaymentProvider.refund()` exists yet), a minimal admin nav shell alongside the orders pages (nothing existed under `(admin)` beyond P1.S6's bare MUI/RTL demo wrapper), `requireStaff()` matching every other admin route's convention. New `orders.admin.*` module (deliberately separate from the customer-facing, `userId`-scoped `orders.service.ts`), `(admin)` layout gained a real server-side auth+role gate, new `AdminToaster.tsx` (MUI `Snackbar`, since the shop's Tailwind-styled `<Toaster/>` resolves to nothing under `(admin)`'s excluded CSS scope). **A real, extensively investigated bundle-budget regression, not glossed over**: real MUI content in `(admin)` pushed every `(shop)` route's First Load JS up ~11KB, traced conclusively to webpack's own automatic chunk-splitting reshuffling once the module graph grew (confirmed via direct chunk-content grepping — zero literal MUI code in any shop-route chunk). Two targeted fixes failed (`optimizePackageImports`, then a forced `enforce:true` MUI cache-group isolation); asked the owner directly rather than keep digging or ship silently — **accepted as a documented tradeoff**, landing now 8KB over its 180KB §10 budget. **Three real bugs found and fixed at their actual root cause during live verification, none papered over**: a genuine React "Only plain objects can be passed to Client Components" crash from a Server Component passing MUI's `ListItemButton` a `component={Link}` reference (fixed by making `AdminShell` a Client Component); a real WCAG list-structure violation from skipping MUI's own `List`→`ListItem`→`ListItemButton` composition; a real heading-order violation from `Typography variant="subtitle1"`'s default `<h6>` mapping jumping past the page's own `<h2>` (fixed with explicit `component="h3"` on all five section headers, same style-vs-semantics separation already used elsewhere). Two MUI `Select` elements also had no accessible name — fixed with `inputProps={{"aria-label": ...}}`. A live debugging detour worth remembering: a "customer redirected to login instead of home" failure that looked like a framework bug was actually a bug in the throwaway verification seed script itself (forgot `name`, a required `meSchema` field) — traced by adding temporary logging rather than assumed. 401/401 backend tests (13 new). Route budgets: `/admin/orders` 340KB, `/admin/orders/[id]` 217KB — no §10 budget applies (internal tool, not customer-facing). See `docs/decisions/0019-p8s1-admin-order-management.md` and §13's Phase 8 entry for full detail.
- 1.20 — **P7.S4 (My Garage management page) shipped — Phase 7's original three-piece scope is now complete.** The last of address-book/wishlist/My-Garage. A real, deliberate departure from every other Phase 7 page: the Garage (`stores/garage-store.ts`, P4.S3) is guest-accessible, cookie-backed client state (masterPlan §3.4), so this page needed zero backend changes and no auth gate at all. Reuses the existing `VehicleSelector` cascade (already writes to the store) via its lazy-loaded wrapper, gated behind an "add vehicle" toggle. Each saved vehicle renders as a real `Card` with a genuine active/inactive visual distinction rather than another flat bordered-row list. `AccountNav` grew a fourth entry and was refactored from four copies of the same JSX into one data-driven map — its own P7.S3 comment had predicted this exact moment. **Two real bugs, both caught live, not assumed fixed**: the same `FORMATTING_ERROR` bug class already documented in this project's history (P6.S6's `codeHelper`/`stockIssue`) — `Garage.itemCount`'s `{count}` needed ICU single-quote escaping since the real count is only known client-side, unlike `/orders`/`/wishlist` which resolve it server-side; and a manual `.replace()` interpolation using plain `String(count)` instead of `toPersianDigits()`, leaking ASCII digits into an all-Persian UI. Verified live (Playwright, ad hoc specs + a11y spec, all deleted after passing): a guest with zero cookies sees the empty state, adds a real vehicle through the live cascade, the new vehicle activates immediately, a second addition deactivates the first (reselectable), removal updates the count correctly, cross-nav to the other three account pages is present even for a guest. axe 0 violations light/dark, empty and populated states. A fresh production build with the API killed still serves a genuine 200 — this route has zero backend dependency, confirmed not assumed. 414/414 backend tests unchanged. `/garage`: 148KB First Load JS. See §13's Phase 7 entry for full detail.
- 1.23 — **P8.S4 (admin categories/brands/attributes CRUD UI) shipped**, closing the last script-only corner of the catalog: these three entities have had admin API endpoints since P3.S1 with no frontend consumer for seven months. Three routes named by §5's own admin map, under one "کاتالوگ" nav entry with an in-page tab bar; list + Dialog rather than the list/new/[id] trio, since these forms are 5 fields, not 15. **Scope extended with the owner mid-step on a real finding**: the attribute dictionary was empty *and unfillable* (0 of 322 products carried any attribute value — P8.S2 scoped attribute values out of the product form), so attributes CRUD alone would have shipped an inert screen; the product form now has a real attribute editor, server-validated against the dictionary, turning on the PLP facets and PDP specs table P5.S1/P5.S2 built and never had data for. **Three real defects fixed at their root, two of them pre-existing and unrelated to this feature**: (1) Mongoose's `countDocuments` is not covered by the soft-delete plugin's `pre(/^find/)`, so *every* paginated endpoint's `meta.total` has counted soft-deleted rows since P2.S2 — proved empirically against a real database, then fixed once in `models/plugins.ts`, red test first. (2) The three delete endpoints had zero referential checks despite `products.service.ts`'s own comment claiming otherwise: deleting a brand silently orphaned every product pointing at it. Now a 409 naming the blocker and its count in Persian, plus restore + a `state=deleted` view — without which a soft delete is irreversible *and* keeps the unique slug index occupied. (3) MUI's `Tooltip` defaults to replacing its child's accessible name, so the categories delete button announced as its own hint rather than "حذف" (fixed with `describeChild`; axe does not flag it). Also caught pre-merge: `sx={{ ps: … }}` is silently dropped by MUI System 7, which would have rendered the hierarchy indent flat. **Two axe findings were investigated and confirmed NOT real** — MUI's own unnamed DataGrid loading overlay, and a 1.1:1 contrast reading that was purely an artifact of auditing mid-Dialog-fade (real pair: 5.7:1). Verified live through a real phone-OTP superadmin login. 505/505 tests (46 new). See `docs/decisions/0023-p8s4-admin-catalog.md` and §13's Phase 8 entry.
- 1.22 — **P8.S3 (admin coupon CRUD + wholesale-account-flag UI) shipped.** Retires the last two "script-only, stays Phase 8 scope" promises this codebase was still carrying: coupon creation (P6.S7's `pnpm create-coupon`) and the wholesale flag (P6.S1's `pnpm set-account-type`). New `coupons.admin.*` and `users.admin.*` modules at `/api/v1/admin/coupons` and `/api/v1/admin/customers`, plus `packages/schemas/src/admin-coupon.ts` and `admin-customer.ts` as their own files from the start (P8.S1's bundle lesson, applied proactively for the second step running). New pages `/admin/discounts` (list/create/edit/deactivate) and `/admin/customers` (searchable list + inline account-type toggle), and two new admin nav entries. **Deactivation is an `endsAt` transition, not a new `status` field and not the soft-delete plugin's `deletedAt`** — `validateCoupon` already rejects an elapsed `endsAt`, so this turns a code off through the one existing enforcement path rather than adding a second parallel notion of "off"; `deletedAt` would additionally have hidden the coupon from this very admin list, destroying the redemption record staff audit against. The cross-field rules the old script never enforced (`percent <= 100`, `endsAt > startsAt`) are checked on the create schema *and* against the merged document on update — not duplication, since a PATCH body alone genuinely cannot answer either question; both sites share the same exported predicates and message constants. **Two real bugs, both found by live queries against the seeded database after the suite was already green**: (1) phone search anchored its regex and so matched only *trailing* fragments — it passed every test written for it while returning zero results for staff typing "0912…", which is how they actually search; fixed to an unanchored substring match with regression tests for leading/middle/trailing fragments. (2) the customers list shipped the whole user document (addresses, garage, wallet balance, login timestamps) for a screen that renders four fields — `passwordHash` was never at risk (`select: false`), but the rest was needless PII on the wire; fixed with an explicit projection plus a test asserting the exact key set. `Coupon.scope` stays unpopulated and unread — product/category scoping remains unimplemented and no scope field appears on the form. 459/459 backend tests (32 new). Route budgets: `/admin/discounts` 341KB, `/admin/customers` 343KB, `/admin/discounts/[id]` 217KB, `/admin/discounts/new` 210KB — no §10 budget applies (internal tool). See `docs/decisions/0022-p8s3-admin-coupons-customers.md` and §13's Phase 8 entry for full detail.
- 1.21 — **P8.S2 (admin products CRUD + inventory adjust UI) shipped.** The catalog was 100% seed-script-only until now. Owner-confirmed scope before writing code: essential fields only (name, slug, sku, brand, category, price + optional wholesale, tax rate, stock, weight, one-line authenticity) — dimensions/warranty/SEO get real, honest server-side defaults rather than crowding the form; media/variants stay out of scope entirely (no upload path or variant model exists). New `products.admin.*` module, `packages/schemas/src/admin-product.ts` kept as its own file from the start — applying P8.S1's own bundle-budget lesson proactively this time instead of rediscovering it. "Archive" is a `status` transition, not the soft-delete plugin's `deletedAt` mechanism. The existing (P3.S6) `POST /admin/inventory/adjust` finally got its first real frontend consumer — a stock-adjust panel on the product edit page. **Two real bugs, both fixed at their actual cause**: (1) create/update use `.create()`/fetch-then-`.save()`, never `findOneAndUpdate` — applying `Product.ts`'s own documented lesson from a real P5.S3 bug (`searchText` silently staying empty) proactively this time. (2) **A real, pre-existing gap across the whole admin CRUD surface, fixed once at the shared root**: every admin create endpoint with a unique index has been falling through the global error handler's generic 500 on a duplicate since categories/brands/attributes shipped — zero clean 400 anywhere. Fixed in `middleware/error.ts` itself, benefiting every existing and future admin route, not patched locally. A live test flake (a fresh unique index still mid-build under `pnpm test`'s full parallel run) fixed with `ProductModel.init()`, not an arbitrary sleep. Verified live (Playwright, ad hoc specs + a11y spec, all deleted after passing): the list shows the real 320-product seeded catalog, a full create→stock-adjust→archive round trip works against real brand/category dropdown data, axe 0 violations, a fresh production build's API-down smoke test serves a graceful 200. 424/424 backend tests (14 new). Route budgets: `/admin/products` 341KB, `/admin/products/[id]` 212KB, `/admin/products/new` 211KB (no §10 budget applies). `(shop)` routes unchanged from P8.S1's own already-accepted baseline. See `docs/decisions/0021-p8s2-admin-products.md` and §13's Phase 8 entry for full detail.
...
dayjs jalaliday
next-themes
clsx tailwind-merge   (cn(); tailwind-merge stays on v2 -- v3 targets
                       Tailwind v4, rejected in §2.3. Added P11.S2,
...
**v1.27 amendment — the section table above describes the page as first built
(P4.S1–S8) and is superseded by the Phase 9 rebuild's inventory.** The 2026-08-14
live audit (`tasks.md`) found the 15-section page 11,556px tall at 390px before
the footer, with the exploded-view list and the ten-tile system grid repeating
the same destinations. The rebuilt inventory is **11 beats**:

| # | Beat | Change from the table above |
|---|---|---|
| 01 | **Hero — Exploded View v2** | Same signature, new costume: layered transparent part renders separating on scroll instead of flat SVG. Absorbs 03's `SYS-xx` chips. Gains an OEM/SKU code field beside the vehicle selector (the Mechanic's above-the-fold path). |
| 02 | Trust strip | Unchanged scope (§5-02). |
| 03 | Best sellers | Unchanged scope; mobile rail sizing fixed. |
| 04 | Authenticity story | Unchanged scope; staged by a video plate. |
| 05 | Shop by vehicle | Unchanged scope; every link now resolves a **real generation** (`/vehicle/[make]/[model]/[gen]`). |
| 06 | Symptom finder | Unchanged scope. |
| 07 | **Interstitial plate** | New — one full-bleed atmosphere plate, copy on the empty third. |
| 08 | Brand wall | Unchanged scope. |
| 09 | Deals | Unchanged scope (conditional; never a fake timer). |
| 10 | **Closing beat** | New composite: how-it-works compressed to its 4 real steps + support with real contact + one CTA into the vehicle selector. |
| 11 | Mega footer | Unchanged scope. |

**Removed:** *Shop by system* (03) as a standalone grid — absorbed into the hero,
whose part links already cover its destinations one-for-one; *How it works* (11)
as its own scroll-section — folded into the closing beat; *Numbers* (10) — **cut**
until four real figures exist; *Newsletter* (14) — **hidden** until a subscription
backend exists; *Guides teaser* (12) — **hidden** until Phase 9 content lands.
Every removal is reversible and tracked in `tasks.md`'s deferred list. The motion
budget above is unchanged and still binding — the hero remains the one
...
| **P5.S4** ✅ | **Brand pages** at `/brand/[slug]`. Reuses the PLP's filter-bar/grid machinery scoped to one brand instead of one category (`GET /catalog/brands/:slug` already exists; product list already accepts `?brand=`). Brand evidence block uses only real model data: name, country, OEM status, optional description/SEO. | Owner accepted the recommended essential v1 facets: category, price, availability; sorting and cursor pagination remain. Brand is fixed by the route and attributes stay hidden for a cleaner page. Public brand parsing gained the trust fields already on the wire and normalizes an omitted empty `seo` subdocument. Live verified responsive light/dark, axe 0, category navigation, RTL 404. 610/610 tests; 146KB route. See ADR 0026. |
...
| **P7.S3** ✅ | **Wishlist page.** Owner's third and final Phase-7 pick of the original three (address book/wishlist/My-Garage) — and, right as it was picked, the owner rated the recent run of Phase 6/7 UI 1/10, with an explicit instruction to keep moving forward but stop letting design quality slide. This step's concrete answer to that: instead of another bordered-row list (`/orders`, `/addresses`), `/wishlist` reuses `plp/ProductCard.tsx` in the same responsive grid `/search` already established — inheriting that component's real visual investment for free, one fewer flat-list pattern in the app rather than one more. Server-side auth gate, same reasoning as `/orders` (zero page-level interactivity — the grid's cards each manage their own `WishlistButton` state independently). New `lib/fetchers/wishlist.ts:fetchWishlist()` (server-side, cookie-forwarded, mirrors `fetchOrders`'s own result-type shape) alongside the existing client-only `fetchWishlistIds()`. `AccountNav`'s `active` union grew a third value (`"wishlist"`), `/orders`/`/addresses` both updated to pass the new required label. | **A real backend gap, not a frontend workaround**: `GET /me/wishlist` (P5.S7) only ever needed to hydrate product data for the *toggle button's* saved/unsaved state, so `wishlist.service.ts` returned the raw Mongoose product document straight through — no `isWholesalePrice`, and the raw `wholesalePriceRial` field leaking to every viewer regardless of account type, unlike every other product-serving list. Fixed by selecting `+wholesalePriceRial` and routing the response through `pricing.ts`'s `toPublicProductJson(product, accountType)` — the exact same account-aware shaping `/catalog/products`, `/catalog/search`, and related-products already use (P6.S1's own "the one place effective price is decided"). Caught because `wishlistResponseSchema.safeParse()` failed silently client-side until traced back to the missing field — a real reminder that a schema mismatch on the frontend is often a sign the backend never finished a shape, not a frontend bug. `EmptyState` (`components/primitives`, 15 existing call sites) gained an optional `icon` slot — additive, backward-compatible, all existing callers unaffected — used here with the same heart glyph `WishlistButton` already renders, a small deliberate first move on the new design-quality mandate rather than a token-only page. Verified live (Playwright, ad hoc specs + a throwaway seed script, all deleted after passing): the grid renders all saved products with cross-nav to `/orders`/`/addresses`, removing a saved product (via the API, confirmed via reload) drops it from the grid, an unauthenticated visit redirects to `/auth/login?next=/wishlist`, the empty state shows the heart icon + browse CTA, axe 0 violations light/dark on both the populated and empty states, and a fresh production build's API-down smoke test serves a graceful 200 even for an unauthenticated visitor (can't distinguish "not signed in" from "API unreachable" without a live 401, same fallback `fetchOrders` already established). 401/401 backend tests (2 new — the shape fix, and a wholesale-account price-resolution case). Route budget: `/wishlist` 127KB First Load JS, matching `/search`'s own footprint exactly. |
| **P7.S4** ✅ | **My Garage management page — the last of Phase 7's original three picks.** Unlike address book/wishlist (both server-persisted, auth-only), the Garage (`stores/garage-store.ts`, P4.S3) is a guest-accessible, cookie-backed client store — masterPlan §3.4's own "guests get a garage too" — so this page needed **zero backend changes and no auth gate at all**, a real, deliberate departure from every other Phase 7 page. Pure client component reading/writing the existing store directly; the existing `VehicleSelector` (Header's own make→model→year cascade, already writes to the store on save) is reused unmodified via its lazy-loaded wrapper, gated behind an "add vehicle" toggle rather than always mounted, same code-splitting reasoning `VehicleSelectorLazy.tsx` already documents. Each saved vehicle renders as a `Card` with a real active/inactive visual distinction (accent ring + brand `Badge` vs. a plain "set active" button) instead of another flat bordered-row list. `AccountNav` grew a fourth entry — its own P7.S3 comment predicted this exact moment, so the four now-duplicated JSX blocks were refactored into one data-driven map, a real DRY improvement now that the repetition is real, not speculative. | **A real bug, the same class already documented in this project's own history (P6.S6's `Auth.login.codeHelper`/`Cart.stockIssue`) — hit again despite being a known pattern**: `Garage.itemCount`'s fa.json string (`"{count} خودرو"`) was passed to a client component as a raw template for manual `.replace()` interpolation (the real count is only known client-side, from the Zustand store — unlike `/orders`/`/wishlist`, which resolve their own counts server-side), but calling `t("itemCount")` without supplying `count` throws next-intl's `FORMATTING_ERROR` the instant it sees unescaped `{count}` ICU syntax. Fixed with the exact same ICU single-quote escaping (`'{count}' خودرو`) already established for `codeHelper`/`stockIssue` — caught live (a real browser console error, not a lint/type error) rather than assumed fixed by pattern-matching the past fix without re-verifying. A second, separate real bug in the same step: the item-count pill's manual interpolation used plain `String(count)` instead of `toPersianDigits()`, rendering ASCII digits inside an otherwise all-Persian UI — caught by a Playwright locator expecting `۲ خودرو` and getting nothing, not by inspection. Verified live (Playwright, ad hoc specs + a11y spec, both deleted after passing): a guest (zero cookies) sees the empty state, adds a real vehicle through the live make→model→year cascade against actual seeded data, the newly-added vehicle becomes active immediately, adding a second vehicle deactivates the first (reselectable), removal drops the count correctly, cross-nav to `/orders`/`/addresses`/`/wishlist` is present even for a guest (those links themselves still gate on click, as they should). axe 0 violations light/dark, empty and populated-with-form-open states. A fresh production build with the API process killed still serves a genuine 200 (this route has literally zero backend dependency, confirmed rather than assumed). 414/414 backend tests unchanged (no backend surface touched). `/garage` route: 148KB First Load JS. |
...
Phase 8's scope above is still directionally locked, to be re-specced when each piece's own turn comes (§16), run the same "one piece at a time" way as Phase 5/6/7. One step ahead of it is real and shipped:

| Step | Deliverable | DoD / notes |
|---|---|---|
| **P8.S1** ✅ | **Admin order management — list, detail, manual status update.** The most urgent Phase 8 gap: staff had zero visibility into orders/payments — a paid order could not be marked shipped/delivered/refunded by anyone. Scoped tightly with the owner before writing code: **status workflow only** (list + detail + manual status update, appending to `statusHistory`) — refunds and invoices explicitly deferred (no `PaymentProvider.refund()` exists yet, a real separate chunk of work); **a minimal admin nav shell** ships alongside the orders pages themselves, since nothing existed under `(admin)` beyond the bare MUI/RTL wrapper from P1.S6; **`requireStaff()`** (any staff role), matching the existing convention every other admin route (`catalog.admin`, `inventory.admin`) already uses — no finer-grained "warehouse"/"fulfillment" role exists yet to justify a narrower check. New `orders.admin.{schema,service,controller,routes}.ts` mounted at `/api/v1/admin/orders`, deliberately a **separate module** from the customer-facing `orders.service.ts` (that one bakes `userId` into every query filter by design — admin needs the opposite, cross-user by default). New `packages/schemas/src/admin-order.ts` — kept as its **own file**, not appended to `order.ts`, after a real, measured budget regression traced back to exactly that (see the note column). The `(admin)` layout itself now carries the auth+role gate (server-side, cookie-forwarded `fetchMeServer`) for the whole route group — unauthenticated visitors get the same `/auth/login` OTP flow customers use (no separate admin login exists), authenticated non-staff get sent home. New `AdminToaster.tsx` (MUI `Snackbar`/`Alert`, reading the same shared `stores/toast-store.ts`) since the shop's own `<Toaster/>` is styled with raw Tailwind classnames that resolve to nothing under `(admin)`'s excluded CSS scope. | **A real, thoroughly investigated bundle-budget regression, not glossed over**: adding real MUI content to `(admin)` pushed every `(shop)` route's First Load JS up ~11KB — confirmed via actual production builds, chunk-by-chunk, with the extra bytes traced to Next.js's own automatic webpack chunk-splitting reshuffling its shared-chunk boundaries once the module graph grew, **not** literal MUI code leaking into shop bundles (verified: grepped every chunk a shop route actually references, zero "mui" string matches). Two targeted fixes attempted and both failed to eliminate it (`experimental.optimizePackageImports`, then a forced `enforce:true` webpack `cacheGroups` isolation for `@mui`/`@emotion`) — asked the owner directly rather than silently ship or silently keep digging indefinitely; **accepted as a documented tradeoff**: landing sits 8KB over its own 180KB §10 budget, every other shop route retains real margin. **Three real bugs caught only by live verification, each fixed at its actual root cause**: (1) `AdminShell` rendering MUI's `ListItemButton` with `component={Link}` from a Server Component crashed with a genuine "Only plain objects can be passed to Client Components" error — React can't serialize a component-type reference across that boundary the way it can a rendered element; fixed by making `AdminShell` itself a Client Component, same pattern the MUI-heavy content components already needed. (2) A real WCAG list-structure violation (`<ul>` with a bare `<a>` direct child) from skipping MUI's own documented `List`>`ListItem`>`ListItemButton` composition; fixed by adding the `ListItem` wrapper. (3) A real heading-order violation (`Typography variant="subtitle1"` defaults to `<h6>`, jumping straight from the page's own `<h2>`) on the order detail page's five section headers; fixed with explicit `component="h3"` on each, same "visual style separate from semantic tag" pattern already used elsewhere in this codebase. Also genuinely caught and fixed: two MUI `Select` elements (status filter, status-update) had no accessible name at all (`aria-input-field-name` axe violation) — fixed with `inputProps={{"aria-label": ...}}`. 401/401 backend tests (13 new). Route budgets: `/admin/orders` 340KB, `/admin/orders/[id]` 217KB (no established §10 budget for admin routes — internal tool, not customer-facing, budget discipline applies to `(shop)` only). |
| **P8.S2** ✅ | **Admin products CRUD + inventory adjust UI.** The catalog was 100% seed-script-only — no staff UI to add/edit/deactivate a product, or adjust stock outside a raw script call. Owner-confirmed scope before writing code: **essential fields only** (name, slug, sku, brand, category, price + optional wholesale, tax rate, stock, weight, one-line authenticity) — dimensions/warranty/SEO get real, honest server-side defaults rather than crowding the form, editable later if a full-detail admin step is ever built; media/variants stay entirely out of scope (no upload path or variant model exists). New `products.admin.{schema,service,controller,routes}.ts` mounted at `/api/v1/admin/catalog/products`, same `requireStaff()`+`auditLog` convention every other admin route already uses. New `packages/schemas/src/admin-product.ts` — its own file from the start this time, applying P8.S1's own bundle-budget lesson immediately rather than rediscovering it. "Archive" is a `status` transition (draft/active→archived), not the soft-delete plugin's own `deletedAt` mechanism — two different concepts, `Product.status` already exists for exactly this. Inventory: reused the existing (P3.S6) `POST /admin/inventory/adjust` — it had zero frontend consumer anywhere until this step; a small stock-adjust panel (delta + reason) on the product edit page is its first real UI. New admin nav entry ("محصولات"). | **Two real bugs found and fixed at their actual cause, both genuinely new (not repeats of a documented pattern)**: (1) `createProduct`/`updateProduct` use `.create()`/fetch-then-`.save()`, never `findOneAndUpdate` — `Product.ts`'s own doc comment on `computeProductSearchText` already documents a real bug this exact mistake caused once before (`seed/catalog.ts`, found at P5.S3): the `pre("save")` hook that computes `searchText` only fires on document middleware, never query middleware. Applied the lesson proactively this time, verified with a dedicated test asserting real `searchText` content after both create and a name-changing update. (2) **A real, pre-existing gap found and fixed at the shared root, not worked around locally**: every admin create endpoint with a unique index (categories.slug, brands.slug, attributes.key, and now products.slug/sku/verificationCode) fell through the global error handler's generic 500 on a duplicate — zero clean 400 anywhere in this codebase for a MongoDB duplicate-key error. Fixed once in `middleware/error.ts` (a real Persian message naming the duplicate field), benefiting every existing and future admin CRUD route, not just this one. A live test-infrastructure flake caught and fixed along the way, worth remembering for any future test asserting a fresh unique-index violation: under `pnpm test`'s full parallel run (not when the file runs alone), a brand-new collection's index can still be mid-build when a rapid second write follows the first — `ProductModel.init()` (resolves once index building completes) fixed it at the actual cause, not papered over with an arbitrary sleep. Verified live (Playwright, ad hoc specs + a11y spec, all deleted after passing): the list shows the real 320-product seeded catalog, a full create→stock-adjust→archive round trip works end to end against real brand/category dropdown data, axe 0 violations on both the list and the create form. A fresh production build with the API killed still serves a graceful 200 (redirects to login, same shared `(admin)` layout gate P8.S1 already established). 424/424 backend tests (14 new: 10 products + the error-middleware coverage implicit in the duplicate-slug case). Route budgets: `/admin/products` 341KB, `/admin/products/[id]` 212KB, `/admin/products/new` 211KB — no §10 budget applies (internal tool). `(shop)` routes unchanged from P8.S1's own already-accepted baseline (no new regression). |
| **P8.S3** ✅ | **Admin coupon CRUD + wholesale-account-flag UI.** The last two features this codebase still ran script-only by explicit earlier decision — coupons (P6.S7's `create-coupon`, create-only, no list/edit/off switch and no validation beyond the type enum) and the wholesale flag (P6.S1's `set-account-type`). Both named Phase 8 as where the real UI would land; this is that step. New `coupons.admin.{schema,service,controller,routes}.ts` at `/api/v1/admin/coupons` and `users.admin.*` at `/api/v1/admin/customers`, each with the same `requireAuth`+`requireStaff()`+`auditLog` stack every admin router uses. `packages/schemas/src/admin-coupon.ts` + `admin-customer.ts` written as their own files from line one (P8.S1's bundle-budget lesson, now applied proactively two steps running). Frontend: `/admin/discounts` list (code search, active/inactive filter, server pagination) + create + edit-with-deactivate, and `/admin/customers` (phone search, account-type filter, inline retail/wholesale toggle). Route name follows §5's own admin map (`/admin/discounts   coupons & campaigns`), not re-invented. The customers screen states out loud that `accountType` is baked into the JWT at issue time, so a flipped flag does not change what that customer is charged until their token refreshes — the more comfortable alternative would have been implying it is instant. Deliberately *not* the full customers module: nothing here can touch a password, phone, or role, and "Customers list/detail" stays its own later item (which now only needs the detail view). | **Deactivation is an `endsAt` transition** — `Coupon` has no `status` field, and `validateCoupon` already rejects an elapsed `endsAt`, so setting it to now stops the code through the one existing, already-tested path instead of adding a second notion of "off" every future validation site would have to remember. `deletedAt` was rejected for a different reason: the soft-delete plugin's own `pre(/^find/)` filter would hide the coupon from this very admin list, destroying the `usedCount`/order-snapshot record staff audit against. Same shape as P8.S2's "archive is a status transition, not a soft delete." **Cross-field rules live in two places on purpose**: `percent <= 100` and `endsAt > startsAt` are on the create schema (whole object present, Zod attaches the error to the right field path) *and* re-checked against the merged document in the service on update — a PATCH body alone genuinely cannot answer either (`{value: 500}` is invalid only if the *stored* coupon is percent-typed). Both call the same exported predicates and message constants, so they cannot drift. **Two real bugs, both surfaced by live queries against the seeded database after the whole suite was green — not by the tests**: (1) the phone search anchored its regex (`` `${national}$` ``), which passed every test written for it because those used complete numbers and a trailing fragment, while returning *zero* results for `?phone=0912` against a database full of `+98912…` customers — staff search by typing the start of a number; fixed to an unanchored substring match, with regression tests specifically for leading, middle and trailing fragments. (2) the customers list serialized the entire user document — addresses, garage, `walletBalanceRial`, `lastLoginAt` — for a screen that renders four fields; `passwordHash` was never exposed (`select: false` on the model) but the rest was needless PII on the wire, fixed with an explicit projection plus a test pinning the exact key set so it cannot silently grow back. Verified live against a running dev server with a real seeded superadmin session: full create → duplicate-rejected (clean 400, not a 500) → percent-over-100-rejected → list-active → deactivate → drops out of active but still listed as inactive with `usedCount` intact; all three new pages render 200 with real Persian content and the new nav. `Coupon.scope` remains unpopulated and read by nothing — product/category scoping is still unimplemented and the form does not pretend otherwise. The two scripts stay in the repo: still the right tool for seeding and headless deploys. 459/459 backend tests (32 new). Route budgets: `/admin/discounts` 341KB, `/admin/customers` 343KB, `/admin/discounts/[id]` 217KB, `/admin/discounts/new` 210KB — no §10 budget applies (internal tool). |
| **P8.S4** ✅ | **Admin categories / brands / attributes CRUD UI — plus the product-attribute link that makes attributes mean anything.** These three entities had full admin API endpoints since P3.S1 and, in the seven months since, not one line of frontend ever called them: staff could not add a brand or rename a category without a script. New pages `/admin/categories`, `/admin/brands`, `/admin/attributes` (§5's own admin route map names all three), reached through a single "کاتالوگ" nav entry plus an in-page tab bar — seven top-level sidebar items would have buried the ones staff use daily, while three separate routes keep each screen's own page/search state in its own URL. Small, flat entities (10 categories, 15 brands), so each screen is a list plus an MUI `Dialog`, not the `list` + `/new` + `/[id]` route trio products and discounts use — that shape is right for Product's 15-field form, not for a 5-field one, and would have meant nine routes. New `categories.admin.*` and `brands.admin.*` modules split off the shopper-facing services (mirroring `products.admin.*` since P8.S2); attributes stay one module, being admin-only end to end already. `packages/schemas` gains `admin-category.ts`, `admin-brand.ts`, `admin-attribute.ts` as their own files from line one — P8.S1's bundle lesson, now applied proactively three steps running. **Scope was extended with the owner after a real finding**: the attribute dictionary was empty (0 rows) and *unfillable* — 0 of 322 products carried a single attribute value, because P8.S2 scoped attribute values out of the product form and `seed/catalog.ts` writes `attributes: []`. Attributes CRUD alone would therefore have shipped a screen that visibly does nothing. The owner chose to close the loop, so the product form gains a real attribute key/value editor, every pair validated server-side against the dictionary (undefined key rejected, select value must be one of its own options, no duplicate keys) — turning on the PLP attribute facets and PDP specs table that P5.S1/P5.S2 built and that have never had data. | **A real, cross-cutting data bug found and fixed at the root, unrelated to the feature itself**: Mongoose's `countDocuments` is its own query-middleware name and is NOT matched by the soft-delete plugin's `pre(/^find/)` hook. `utils/pagination.ts` pairs `find` with `countDocuments` over the same filter, so **every paginated endpoint in the app has been reporting a `meta.total` that counts soft-deleted rows its own `data` array excluded** — since P2.S2. Proved empirically against a real database before touching anything (`find()` = 2, `countDocuments()` = 3), then fixed once in `models/plugins.ts` with a red-first test; deliberately `/^(find|countDocuments)/` and not `/^count/`, since `estimatedDocumentCount` takes no filter and would throw. **The three delete endpoints had no referential check at all** — `products.service.ts`'s own comment already claimed "admin CRUD guards deletion of a category/brand still in use", and it simply did not. Deleting a brand would have silently orphaned every product pointing at it; a category would additionally 404 its own PLP while its products stayed live and unbrowsable. Now a clean 409 naming the blocker and its count in Persian digits, plus `POST /:id/restore` and a `state=deleted` filter — without which a soft delete is irreversible *and* keeps the unique slug index occupied, so re-creating the same slug fails with a duplicate-key 400 and no way to see why. Category re-parenting gained cycle and has-children guards, and a slug rename now cascades into descendants' materialized `path` (one positional `updateMany`; slugs are unique so at most one element matches) — the gap `updateCategory`'s own P3.S1 comment admitted. **A real a11y defect caught only because a name-based locator could not find a button**: MUI's `Tooltip` defaults to becoming its child's accessible *name*, so the categories delete button announced as "این دسته‌بندی در حال استفاده است" instead of "حذف" — the hint replacing the action; fixed with `describeChild`. axe never flagged it (the button still had *a* name). Also caught pre-merge: `sx={{ ps: ... }}` is silently dropped — MUI System 7 registers only `pl`/`pr` as spacing shorthands — so the category hierarchy indent would have rendered flat; fixed with explicit `paddingInlineStart` (`pl` being both physical, against rule 6, and the wrong edge in RTL). **Two axe findings investigated and confirmed NOT real**, rather than "fixed": an unnamed `role="progressbar"` (MUI's own DataGrid loading overlay, present on every admin grid since P8.S1) and a 1.1:1 contrast reading on helper text whose real colour pair is 5.7:1 — the latter purely an artifact of auditing during the Dialog's ~225ms fade, confirmed by polling computed opacity to completion rather than sleeping. Verified live end to end against a running dev server with a real phone-OTP superadmin login (not an injected cookie, per P7.S3's own warning): real derived counts on screen, a delete genuinely refused with the counted Persian message and the row still present, a full brand create→edit→delete→restore round trip, a new attribute defined and then actually offered on the product form, axe 0 violations on all three screens and with a dialog open. Fresh production build with the API process killed still serves graceful 200s on all three routes. 505/505 tests (46 new). Route budgets: `/admin/categories` 356KB, `/admin/brands` 355KB, `/admin/attributes` 356KB — no §10 budget applies (internal tool); every `(shop)` route unchanged from P8.S1's already-accepted baseline. |

### PHASE 9 — Landing rebuild, content, SEO, hardening
...
**The phase opened with a storefront-design track that grew into a full landing rebuild.** P9.S1 revived the visual direction (v1.26); the 2026-08-14 live audit then found seven real defects (`tasks.md`), and the owner supplied a set of rendered 3D assets. The rebuild plan came from an external planning model (Fable 5) working off `docs/landing-rebuild-brief.md`, and is adopted as `fableTasks.md` — **that file is the step-level plan of record for S2–S17**; this table is the summary and the amendment layer. Content/SEO/hardening work above resumes after S17.

| Step | Deliverable |
|---|---|
| **P9.S1** ✅ | **Storefront visual direction revived** — "Persian workshop manual" established, `apps/web/design-quality.md` restored as the maintained checklist, header + hero + trust/system/authenticity cadence rebuilt away from equal white cards. |
| **P9.S2** ✅ | **Asset hygiene.** The 28 supplied masters (113MB) moved out of `apps/web/public/Landing/` into a git-ignored `landing-src/`, split `cutouts/` · `plates/` · `video/` · `samples-opaque/` and renamed semantically; `apps/web/public/landing/` is left for S3's optimized output, so no master ever enters git history. Inventory, provenance and usage rules in **`docs/landing-assets.md`** (the plan said a README under `public/` — moved to `docs/` because `public/` is served verbatim and a README there would ship to users). Each name verified by opening the file: `lightning.png` is a sealed-beam headlight, the four `hf_*` plates are overhead / front / engine / body, and the ten part renders are **real** cutouts (alpha decoded: 74–90% transparent, corner alpha 0). Two plan corrections recorded: **no grille render exists**, and the vehicle is a classic fastback coupe. |
| **P9.S3** | **Asset pipeline.** `sharp` (manifest-approved) + `scripts/optimize-landing.mjs` → AVIF/WebP responsive sets, video posters, re-encoded clips. Only optimized outputs are committed. |
| **P9.S4** | **Strings.** `fa.json` `Landing` namespace v2 — real Persian for all 11 beats; §5's specified copy is canonical where it exists. String table presented to the owner for review. |
| **P9.S5** | **HeroV2 scaffold** behind `NEXT_PUBLIC_LANDING_V2` — layered cutouts, scroll-driven separation in three chapters, leader lines + mono `SYS-xx` + **real** counts from `getSystemPartCounts`, vehicle selector + OEM/SKU field composed inside. |
| **P9.S6** | **HeroV2 proof + flip.** RTL, keyboard, axe 0, LCP re-measured with the car cutout as the LCP element; flag defaulted on; legacy hero retained as reference until S16. |
| **P9.S7** | Trust strip — hairline evidence rail, every claim tied to a concrete process. |
| **P9.S8** | Best-sellers rail — definite card sizing inside the snap rail (**audit item 2**), with a regression assertion. |
| **P9.S9** | Absorb Shop-by-system into the hero; destinations verified 1:1 before the standalone grid is removed (**audit item 4**). |
| **P9.S10** | Shop-by-vehicle links resolve real generations (**audit item 1**), with a route-level test walking every rendered href. |
| **P9.S11** | Authenticity story staged by video, telling the Authenticity Record with a **real** example product. |
| **P9.S12** | Symptom finder + the new interstitial plate beat. |
| **P9.S13** | Brand wall (pause on hover + reduced motion) and Deals (renders nothing when no live deals exist). |
| **P9.S14** | **Closing beat** + real contact data + hides (**audit items 5 & 6**): 4-step rail, support, one CTA; Numbers cut, Newsletter/Guides behind explicit flags. |
| **P9.S15** | Closing ambience + footer pass (e-Namad slot, contact parity, column links resolve). |
| **P9.S16** | **Regression suite** (**audit item 7**) — Playwright screenshots 360/390/1440 × light/dark × reduced-motion, href assertions, card width, axe 0 stays 0. Legacy hero deleted here. |
| **P9.S17** | Measure + hand off — route JS and LCP re-measured per `docs/performance-landing.md` methodology, that doc updated, `tasks.md` deferred list refreshed. |

**Binding amendments to the adopted plan** (recorded here so the step file is not the only source):
1. **No new runtime dependency.** `sharp` is manifest-approved (§4) and installs in S3. `three`, `@react-three/fiber`, `@react-three/drei`, GSAP, Lenis and `model-viewer` remain **out** — a WebGL v2 beat is an explicit future owner decision, costing ≈150KB gz plus a manifest amendment plus a budget renegotiation.
2. **Bundle budget.** The route's accepted 188KB gz figure is the ceiling to hold, not a floor to build on; the rebuild's own net-new client code is offset by retiring the legacy Exploded View client bundle and three removed sections. Re-measure **before** each JS-touching merge.
3. **`ffmpeg` is a local tool, not a repo dependency.** Installed at S3 (Gyan.FFmpeg 9.0, winget, owner-approved 2026-08-20) and used only by `scripts/optimize-landing.mjs`, which is run by hand. Nothing in build, test or CI touches it, and the script still emits the image set when it is absent.
4. **Asset reality check, verified 2026-08-20:** the ten part renders are genuine transparent cutouts; there is **no grille render** (the parts are car, headlight, bumper, piston, alternator, air filter, door, hood, fender, windshield); `car.png` is a classic fastback coupe, shipped as workshop atmosphere with a domestic-sedan regeneration parked as a later drop-in swap.
5. **Imagery law still governs** (`design-quality.md`): these renders are atmosphere and diagram. They may headline a system or an editorial beat; they may never sit in a product card as a catalog photo, and never pose as authenticity or inspection evidence.
