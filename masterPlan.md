...
**Status:** Phases 0–4 complete (P0.S1–P4.S8). Palette refined to v1.5 (Marigold CTA) per owner review. **GATE 4→5 cleared 2026-07-28.** Phase 5 (storefront) is underway, incrementally, by explicit owner choice ("one piece at a time" rather than a single upfront spec) — **P5.S1 (Category/PLP) and P5.S2 (PDP) done and pushed**; §13's Phase 5 table now carries the same per-step depth Phases 0–4 have, backfilled from what was actually built plus drafted ahead for the remaining pieces.
**Document version:** 1.6 — Phases 0–4 are locked. Phase 5 is executing step-by-step (§13); each step here reflects a real, shipped decision, not a plan. Phases 6–10 remain directionally locked, re-specced only when their turn comes.
...
| **P5.S3** | **Search results** at `/search?q=`. The header's search form already submits here (`action="/search"`, `name="q"`) — currently 404s. Results grid over the existing `GET /catalog/search` (Persian-normalized text + prefix + OEM-exact, already built P3.S4), page/limit pagination, empty state with a "did you mean checking spelling / try a system name" hint (no fabricated "did you mean" suggestion algorithm — that needs real query-log data this store doesn't have yet). No live autocomplete/type-ahead dropdown in this pass — that's a separate client-heavy feature (debounced queries, a suggestions endpoint that doesn't exist) with its own budget cost; revisit once there's usage data showing it's worth it. | Same DoD floor as P5.S1/S2 (§14) plus: verify the header form's existing plain-HTML-submit still works with JS disabled (progressive enhancement, not a client-only search box). |
...
| **P5.S7** | **Wishlist** (storefront half — a save/unsave affordance on `ProductCard`/PDP). §9 already lists `GET/POST/DELETE /me/wishlist` but they don't exist yet — needs auth (Phase 2 delivered phone-OTP auth, unused by any storefront feature so far). First real feature requiring a signed-in user; the guest-vs-auth question that Cart (P5.S8) also needs answered gets settled here first, since wishlist is the simpler of the two. | Real open question, don't guess: does a guest get a *local-only* wishlist (parallel to the Garage's cookie pattern) that merges on login, or is wishlist auth-only from day one? The Garage precedent (guest cookie + merge) argues for consistency; but wishlist has real backend persistence needs the Garage doesn't. Decide at step start. |
| **P5.S8** | **Cart** (persistent, guest + auth merge, per this phase's own original scope). `Cart` model already specced (§3.2): `userId? · anonId? · items[] · couponCode? · expiresAt`. Whatever guest-identity mechanism P5.S7 lands on (cookie anonId, most likely) is reused here, not reinvented. Stock reservation on add-to-cart is explicitly **not** this step — `StockReservation` (built ahead of schedule in P3.S6) activates at checkout initiation (Phase 6), so cart itself is a plain quantity ledger against live `stock`, re-validated at every step, never assumed correct from a stale read. | **GATE 5→6** candidate: this is the last Phase 5 piece and the direct dependency Phase 6's checkout needs — confirm cart end-to-end (add/update/remove, guest→auth merge, coupon apply/remove if built, live stock re-validation) before Phase 6 starts. |

**GATE 5 → 6:** Cart works end-to-end (guest and authenticated), coupon apply/remove if in scope, live stock re-validation confirmed. Phase 6 also needs §15 Q3/Q5 (e-Namad/legal status, payment gateway choice) answered — those are blocking questions now, not someday questions; see §15's own status column.
...
| 3 | Legal entity + e-Namad status — started or not? | **Still open. Now urgent** — Phase 5 (this phase) is the one directly before Phase 6 (checkout/payments), and Phase 6 is explicitly blocked on this paperwork (§11's own callout: "start that paperwork during Phase 2 — it takes weeks"). Ask before P5.S8 (cart) finishes, so the answer is in hand when Phase 6 starts, not discovered as a surprise blocker then. |
...
| 5 | Payment gateway preference — Zarinpal, Zibal, IDPay, or a bank PSP? | **Still open.** `.env.example` defaults `PAYMENT_PROVIDER=mock` — no real choice made yet. Blocks Phase 6's real `PaymentProvider` implementation (mock/sandbox can proceed regardless, per §11). Same urgency as #3 — needed before Phase 6 starts, not during it. |
...
| 8 | Do we sell to mechanics at wholesale prices (B2B pricing tiers)? | **Flagging a real miss, not silently patching it:** this document said tiers "must enter the Product model in Phase 3, not be bolted on later" (§3.2) — Phase 3 is complete and closed (GATE 3→4 cleared), and no such field exists on `Product` today (only `priceRial`/`compareAtRial`). If the answer turns out to be "yes," this is now exactly the "bolted on later" scenario the original plan tried to avoid — a real schema migration against live products, not a clean Phase 3 addition. Needs an explicit answer, and if yes, a real migration step should be added to Phase 5 or 6 rather than assumed away. |

**Practical read:** #3 and #5 are the two that actually gate upcoming work (Phase 6) and deserve an answer before P5.S8 wraps up, not after. #2 and #8 are worth a quick answer whenever convenient — cheap to answer now, expensive if discovered as a surprise later (#8 especially, per its own note above). #6 and #7 can wait until their respective phases.
...
**Locale files: identical key sets, verified by diffing the trees, not by eye.** Every step that touches `messages/fa.json`/`en.json` gets a quick `node -e` key-diff check before commit (see any recent step's own verification) — cheaper than discovering drift in CI.
