...
**Status:** Phases 0–4 complete (P0.S1–P4.S8). Palette refined to v1.5 (Marigold CTA) per owner review. **GATE 4→5 cleared 2026-07-28.** Phase 5 (storefront) is underway, incrementally, by explicit owner choice ("one piece at a time" rather than a single upfront spec) — **P5.S1 (Category/PLP), P5.S2 (PDP), P5.S3 (search results), and P5.S7 (wishlist, auth-only, plus the previously-unscheduled `/auth/login` page it needed) done and pushed**; §13's Phase 5 table now carries the same per-step depth Phases 0–4 have, backfilled from what was actually built plus drafted ahead for the remaining pieces.
