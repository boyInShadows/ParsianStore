...
3. **`ffmpeg` is a local tool, not a repo dependency.** It is not currently installed on the dev machine; S3's video work is gated on the owner installing it (or on deferring the video beats). Image work does not depend on it.
