...
**Status:** Phases 0–4 complete (P0.S1–P4.S8). Palette refined to v1.5 (Marigold CTA) per owner review. **GATE 4→5 cleared 2026-07-28.** Phase 5 (storefront) executed incrementally, by the owner's explicit "one piece at a time" choice — **P5.S1 (Category/PLP), P5.S2 (PDP), P5.S3 (search results), P5.S7 (wishlist + `/auth/login`), and P5.S8 (cart) shipped; P5.S4–S6 (brand pages, vehicle landing pages, compare) were deprioritized in favor of cart and remain undone**, per the owner's own ordering, not an oversight — §13's table still carries their draft specs for whenever they're picked up. **GATE 5→6 cleared 2026-07-29**, since its own criteria (cart end-to-end, §15 Q3/Q5/Q8 answered) didn't require S4–S6. §15's three Phase-6-blocking open questions were answered directly with the owner at the end of P5.S8 — see §15 and the Phase 6 section header for what changes as a result (Zarinpal chosen; e-Namad paperwork still not started, external blocker persists; B2B pricing tiers scheduled as new step P6.S1).
**Document version:** 1.7 — Phases 0–4 are locked; Phase 5 is gate-cleared with P5.S4–S6 left as open backlog, not blocking Phase 6. Phase 6 is next, starting with the newly-scheduled P6.S1 (B2B pricing migration) before its originally-planned checkout/payments scope. Phases 7–10 remain directionally locked, re-specced only when their turn comes.
...
> **New first-order item, not originally scoped:** B2B wholesale pricing tiers for mechanics. masterPlan originally required this field on `Product` from Phase 3 (§3.2) — Phase 3 is closed with no such field, which the owner confirmed at the end of P5.S8 is a real miss, not a "someday." Owner's decision: **add it now, as a real schema migration against the live `Product` model**, not bolt it on silently later. This must be **Phase 6's first step (P6.S1)**, before checkout/pricing logic is built on top of a single-price assumption — re-spec this step properly at Phase 6 kickoff (§16 discipline) rather than guessing the field shape here.
